...
export default function PatternMatrix({ rows }) {
  return (
    <div>
      <div className="flex items-center gap-4 px-4 py-2 border-b border-rule bg-stone2 text-[10px] font-body italic text-ghost">
      ...
      {rows.map((row, i) => (
        ...
        >
          <div className="px-4 py-2.5">
            ...
            {row.sub && <div className="font-body text-[10px] italic text-ghost mt-0.5">{row.sub}</div>}
          </div>
          ...
          {row.trend ? (
            <div className="px-3 text-center">
              ...
              {row.trendLabel && <div className="font-body italic text-ghost text-[9px] whitespace-nowrap">{row.trendLabel}</div>}
            </div>
          ) : <div className="px-3" />}
          <div
            className="font-display text-base font-bold italic tracking-tight px-4 text-right"
            style={{ color: row.critical ? '#D94F2A' : row.warn ? '#C4920A' : '#A8A098' }}
          >
            {row.count}
          </div>
        </div>
