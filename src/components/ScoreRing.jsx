  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} role="img" aria-label={`Score: ${score} out of ${maxScore}`}>
      ...
      <text
        x={size / 2} y={size / 2 - 4}
        textAnchor="middle"
        style={{ fontFamily: '"PP Editorial New", Georgia, serif', fontWeight: 800, fontStyle: 'italic', fontSize: size * 0.26, letterSpacing: '-0.02em', fill: color }}
      >
        {score}
      </text>
      <text
        ...
        textAnchor="middle"
        style={{ fontFamily: '"Source Serif 4", Georgia, serif', fontStyle: 'italic', fontSize: size * 0.1, fill: '#A8A098' }}
      >
        {label}
      </text>
    </svg>
