  ...
  return (
    <div className="border-b border-rule last:border-0">
      <div className="flex items-center justify-between px-4 pt-3 pb-2">
        <span className="font-body text-xs italic text-muted">{metric}</span>
        <div className="flex items-center gap-1 px-2 py-0.5 border border-rule text-[10px] font-body italic text-ghost">
      ...
      <div className="flex items-baseline gap-2 px-4 pb-2">
        <span
          className="font-display text-3xl font-bold italic tracking-tight leading-none"
          style={{ color: percentile >= 60 ? '#3A8A5A' : percentile >= 40 ? '#C4920A' : '#D94F2A' }}
        >
          {score}
        </span>
        {delta && (
          <span className={`font-body text-xs italic flex items-center gap-0.5 ${deltaDir === 'up' ? 'text-ok' : 'text-danger'}`}>
            {deltaDir === 'up' ? '↑' : '↓'} {delta}
          </span>
        )}
      </div>
      ...
      <div className="flex items-center gap-2 px-4 pb-3">
        ...
        <span className="font-body text-[10px] italic text-ghost whitespace-nowrap">
      ...
      {peers && peers.length > 0 && (
        <div className="px-4 pb-2 border-t border-rule pt-2">
          <div className="text-[9px] font-body italic uppercase tracking-wide text-ghost mb-2">Top plants · comparable category</div>
          {peers.map((p, i) => (
            <div key={i} className="flex items-center gap-2 py-1 border-b border-rule last:border-0">
              <span className="text-xs">{['🥇','🥈','🥉'][i]}</span>
              <span className="flex-1 font-body text-[11px] italic text-muted">{p.name}</span>
              <span className="font-display text-xs font-bold italic text-ink">{p.value}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {zones && (
        <div className="px-4 py-2.5 border-t border-rule">
          ...
          <div className="flex">
            {zones.map((z, i) => (
              <div key={i} className="flex-1 text-center font-body text-[9px] italic" style={{ color: z.color }}>
                <div>● {z.label}</div>
      ...
      {insight && (
        <div className="px-4 py-2 bg-warn/5 border-t border-rule font-body text-[11px] italic text-warn">
          {insight}
        </div>
      )}
    </div>
