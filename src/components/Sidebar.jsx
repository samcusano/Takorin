...
import {
  ...
  Building2, ChevronRight, Globe2, HardHat, Bell,
  LayoutDashboard
} from 'lucide-react'
...
const foundation = [
  ...
  { id:'operator',       label:'Operator View',         path:'/operator',       icon:HardHat,  badge:null },
...
const accentMap = {
  ...
  supplier:'#8A6A3A', capa:'#C4920A', readiness:'#C17D2A', network:'#C17D2A', operator:'#8A6A3A', notifications:'#C17D2A',
}
...
function Badge({ badge, badgeType }) {
  ...
  if (badgeType === 'score') return (
    <span className="ml-auto text-[10px] font-semibold px-1.5 py-0.5 bg-ochre text-stone font-body italic">
      {badge}
    </span>
  )
  return (
  ...
  return (
    ...
    ">
      ...
      <div className="flex items-center gap-3 px-4 py-3.5 border-b border-sidebar-border bg-sidebar2">
        ...
        <div>
          <div className="font-display font-bold italic text-stone text-base tracking-tight leading-none">
            takorin
          </div>
          <div className="font-body italic text-ghost text-[10px] mt-0.5">
            Total intelligence
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="font-body italic text-ghost text-[10px]">Plant ID SL-04</div>
        </div>
        <ChevronRight
          size={13}
      ...
      {plantOpen && (
        <div className="mx-3 mb-1 border border-sidebar-border bg-sidebar slide-in">
          <div className="flex items-center justify-between px-3 py-2 bg-sidebar2">
            ...
            <span className="font-body italic text-[9px] text-ok bg-ok/10 px-1.5 py-0.5">Active</span>
          </div>
          {['Topeka Plant', 'Denver Plant', 'Wichita Plant'].map(p => (
            <div key={p} className="flex items-center justify-between px-3 py-2 border-t border-sidebar-border opacity-50">
              <span className="font-body italic text-[#A8A098] text-[11px]">{p}</span>
              <span className="font-body italic text-ghost text-[9px]">Not in pilot</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="px-4 py-2.5 border-t border-sidebar-border">
        <div className="flex items-center justify-between">
          <span className="font-body italic text-ghost text-[10px]">Compliance</span>
          <span className={`font-body italic font-medium text-[9px] px-2 py-0.5 ${complianceColor}`}>
            {complianceLabel}
          </span>
        </div>
        <div className="font-body italic text-ghost text-[9px] mt-0.5 leading-relaxed">
      ...
      <div className="flex items-center gap-2.5 px-4 py-3 border-t border-sidebar-border">
        <div className="w-7 h-7 rounded-full bg-ochre flex items-center justify-center flex-shrink-0">
          <span className="font-display font-bold italic text-stone text-[11px]">JC</span>
        </div>
        <div>
          <div className="font-body text-stone text-[12px] font-medium">J. Crocker</div>
          <div className="font-body italic text-ghost text-[10px]">Plant Director</div>
        </div>
      </div>
      ...
      {toast && (
        <div className="fixed bottom-4 left-4 z-50 bg-sidebar border border-sidebar-border px-3 py-2 slide-in">
          <span className="font-body italic text-ghost text-[11px]">{toast} — not available in pilot</span>
        </div>
      )}
    </aside>
  )
