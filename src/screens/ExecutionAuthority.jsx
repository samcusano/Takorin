  ...
  return (
    <button type="button" onClick={onClick}
      ...
      <div className="flex items-center gap-2 mb-0.5">
        ...
        <span className={`font-body font-medium text-[13px] ${tier.color}`}>{tier.label}</span>
        {tier.agentCount > 0 && (
          <span className="ml-auto font-body text-ghost text-[12px]">{tier.agentCount} agents</span>
        )}
      </div>
      <p className="font-body text-ghost text-[12px] leading-snug">{tier.description}</p>
      {tier.actionCount > 0 && (
        <div className="flex items-center gap-3 mt-1.5">
          <span className="font-body text-[12px] text-muted">{tier.actionCount} actions</span>
          {tier.approvalRate != null && (
            <span className="font-body text-[12px] text-ok">{Math.round(tier.approvalRate * 100)}% approved</span>
          )}
          {tier.rollbackRate != null && (
            <span className="font-body text-[12px] text-warn">{Math.round(tier.rollbackRate * 100)}% escalated</span>
          )}
        </div>
      )}
    </button>
    ...
    <button type="button" onClick={onClick}
      ...
      <div className="flex items-start gap-2 mb-1">
        ...
        <div className="flex-1 min-w-0">
          <p className="font-body text-ink text-[12px] leading-snug mb-0.5">{entry.action}</p>
          <div className="flex items-center gap-2">
            <span className="font-body text-ghost text-[12px]">{entry.agent}</span>
            ...
            <span className="font-body text-ghost text-[12px] capitalize">{entry.tier}</span>
          </div>
        </div>
        <span className={`font-body text-[8px] px-1.5 py-0.5 flex-shrink-0 ${out.cls}`}>{out.label}</span>
      </div>
      <div className="flex items-center gap-3 pl-3.5">
        <span className="font-body text-ghost text-[12px]">{entry.timeLabel}</span>
        {entry.monitoringWindow && (
          <span className="font-body text-ghost text-[12px]">· {entry.monitoringWindow} window</span>
        )}
        {!entry.reversible && (
          <span className="font-body text-muted text-[12px]">· Irreversible</span>
        )}
      </div>
    </button>
  )
    ...
    <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
      ...
      <div>
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="font-body text-ghost text-[12px] tracking-normal capitalize">{entry.tier} tier</span>
          <span className={`ml-auto font-body text-[12px] px-1.5 py-0.5 ${out.cls}`}>{out.label}</span>
        </div>
        <div className="font-display font-bold text-ink text-[18px] leading-tight mb-1">{entry.action}</div>
        <div className="font-body text-ghost text-[13px]">{entry.agent} · {entry.timeLabel}</div>
      </div>

      {/* Rationale */}
      <div className="px-4 py-3 bg-stone2 border border-rule2 border-l-4 border-l-ochre">
        <div className="font-body text-ghost text-[12px] tracking-normal mb-1">Agent rationale</div>
        <p className="font-body text-ink text-[13px] leading-relaxed">{entry.rationale}</p>
      </div>
      ...
      {entry.impact && (
        <div className={`flex items-center gap-3 px-4 py-3 border ${entry.impact.positive ? 'border-ok/30 bg-ok/[0.04]' : 'border-warn/30 bg-warn/[0.04]'}`}>
          ...
          <div>
            <div className="font-body text-ghost text-[12px] tracking-normal">{entry.impact.metric}</div>
            <div className={`font-body font-medium text-[15px] ${entry.impact.positive ? 'text-ok' : 'text-warn'}`}>{entry.impact.delta}</div>
          </div>
        </div>
      )}
      ...
      {entry.escalationNote && (
        <div className="flex items-start gap-2 px-3 py-2.5 border border-warn/30 bg-warn/[0.04] border-l-2 border-l-warn">
          ...
          <p className="font-body text-warn text-[13px] leading-snug">{entry.escalationNote}</p>
        </div>
      )}
      ...
      {rb && (
        <div className="border border-rule2 bg-stone2 px-4 py-3">
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <span className="font-body text-muted text-[12px] tracking-normal">Rollback executed</span>
          </div>
          <p className="font-body text-ink text-[13px] mb-1">{rb.rollbackAction}</p>
          <p className="font-body text-ghost text-[12px] leading-snug">{rb.reason}</p>
          <div className="flex items-center gap-2 mt-2">
            <span className="font-body text-ghost text-[12px]">{rb.timestamp.replace('T', ' ').substring(0, 16)}</span>
            <span className="font-body text-ghost text-[12px] opacity-50">·</span>
            <span className="font-body text-[12px] px-1.5 py-0.5 bg-ok/10 text-ok border border-ok/30">Complete</span>
          </div>
        </div>
      )}
    </div>
    ...
    <div className="flex h-full overflow-hidden content-reveal">
      ...
      <div className="w-[260px] flex-shrink-0 border-r border-rule2 flex flex-col bg-stone">
        <div className="flex-shrink-0 px-5 py-4 border-b border-rule2 bg-stone2">
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <span className="font-body text-ghost text-[12px]">autonomous actions · 30d</span>
          </div>
        </div>
        ...
        <div className="flex-shrink-0 px-4 py-2 border-b border-rule2 bg-stone2">
          <div className="font-body text-ghost text-[12px] tracking-normal">Autonomy tiers</div>
        </div>
        <div className="flex-1 overflow-y-auto">
          <button type="button" onClick={() => { setSelectedTier(null); setSelectedId(null) }}
            ...
            <span className="font-body font-medium text-ink text-[13px]">All events</span>
            <span className="font-body text-ghost text-[12px] ml-2">{executionLog.length}</span>
          </button>
          {autonomyTiers.map(t => (
      ...
      <div className="w-[380px] flex-shrink-0 border-r border-rule2 flex flex-col">
        <div className="flex-shrink-0 px-4 py-2.5 border-b border-rule2 bg-stone2 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <span className="flex items-center gap-1 font-body text-ok text-[12px]"><span className="w-1.5 h-1.5 rounded-full bg-ok" />Success</span>
            <span className="flex items-center gap-1 font-body text-warn text-[12px]"><span className="w-1.5 h-1.5 rounded-full bg-warn" />Escalated</span>
            <span className="flex items-center gap-1 font-body text-ochre text-[12px]"><span className="w-1.5 h-1.5 rounded-full bg-ochre" />Pending</span>
          </div>
        </div>
        <div className="flex-1 overflow-y-auto">
          {filtered.map(e => (
        ...
        <div className="flex-shrink-0 px-4 py-3 border-t border-rule2 bg-stone2">
          <div className="flex items-start gap-2">
            ...
            <div>
              <div className="font-body font-medium text-ink text-[12px] mb-0.5">Current authority ceiling: Execute</div>
              <p className="font-body text-ghost text-[12px] leading-snug">Govern tier not yet active. 5 agents operate autonomously within pre-approved action classes.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-stone">
        <div className="flex-shrink-0 px-5 py-2.5 border-b border-rule2 bg-stone2">
          <span className="font-body text-ghost text-[12px] tracking-normal">Event detail</span>
        </div>
