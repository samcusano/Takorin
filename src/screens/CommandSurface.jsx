...
function ModulePill({ label, accent }) {
  return (
    <span className="font-body italic text-[10px] px-2 py-0.5" style={{ color: accent, background: accent + '18' }}>
      {label}
    </span>
  )
}
  ...
  if (isPending) {
    return (
      <div className="border-b border-rule2 border-l-2 border-l-ok px-3 py-2.5 bg-ok/5 flex items-center gap-2">
        ...
        <span className="font-body italic text-ok text-[10px]">Acknowledged</span>
      </div>
    )
  }

  const borderCls    = { danger: 'border-l-danger', warn: 'border-l-warn', watch: 'border-l-rule2' }[item.urgency]
  const actionColor  = { danger: '#D94F2A', warn: '#C4920A', watch: '#5A524C' }[item.urgency]
  ...
  return (
    <div className={`border-b border-rule2 border-l-2 ${borderCls} px-3 py-3 flex flex-col gap-2 ${isWatch ? 'bg-stone2/30' : ''}`}>
      <div className="flex items-center gap-1.5 flex-wrap">
        <span className="font-body italic text-[9px]" style={{ color: item.moduleAccent }}>{item.moduleLabel}</span>
        <span className={`font-body font-medium text-[9px] uppercase tracking-widest ${timeLabelCls}`}>
          {isNow && (
      ...
      <p className="font-body italic text-[10px] leading-snug truncate" style={{ color: actionColor }}>
        ▸ {item.action}
      </p>
      {confirming ? (
        <div className="flex items-center gap-2 pt-1.5 border-t border-rule2">
          <p className="font-body italic text-ink2 text-[10px] flex-1">Acknowledge and remove? You have 6 seconds to undo.</p>
          <button
            onClick={() => setConfirming(false)}
            className="font-body italic text-[10px] px-2 py-1 border border-rule2 text-ink2 hover:border-ghost transition-colors"
        ...
        <div className="flex items-center justify-between pt-1.5 border-t border-rule2">
          <span className="font-body italic text-ghost text-[9px]">{item.owner.name}</span>
          <button
            onClick={handleClick}
  ...
  return (
    <div className="flex flex-col min-h-0 overflow-hidden bg-stone">
      ...
      <div className="flex-1 overflow-y-auto">
        {items.length === 0 ? (
          <div className="px-4 py-8 text-center font-body italic text-ghost text-[11px]">{emptyMsg}</div>
        ) : (
          items.map(item => (
            <CommandCell
              key={item.id}
  ...
  return (
    <div className="border-t border-rule2">
      ...
      >
        ...
        <span className="font-body italic text-[10px] text-muted">
          {items.length} {open ? '▴' : '▾'}
        </span>
      </button>
      {open && items.map(item => (
      ...
      {entries.map(({ id, title }) => (
        <div key={id} className="flex flex-col bg-ink border border-ink2 slide-in overflow-hidden">
          <div className="flex items-center gap-4 px-4 py-2.5">
            <span className="font-body italic text-stone text-[11px] flex-1 min-w-0 truncate">
              Acknowledged: {title}
            </span>
            <button
              onClick={() => onUndo(id)}
        ...
        <div className="w-[220px] flex-shrink-0 border-l border-rule2 overflow-y-auto bg-stone2 flex flex-col">
          ...
          <SP title="By module">
            {Object.entries(moduleCounts).length === 0 ? (
              <div className="px-4 py-3 font-body italic text-muted text-[11px]">No active items</div>
            ) : (
              Object.entries(moduleCounts).map(([label, { count, accent, module }]) => (
                <div key={label} className="flex items-center justify-between px-4 py-2.5 border-b border-rule2 last:border-b-0">
                  <Link to={`/${module}`} className="font-body italic text-[12px] hover:underline" style={{ color: accent }}>
                    {label}
                  </Link>
