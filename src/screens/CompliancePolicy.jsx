...
function StatusBadge({ status }) {
  ...
  return <span className={`font-body text-[12px] tracking-normal px-1.5 py-0.5 ${cfg.cls}`}>{cfg.label}</span>
}
    ...
    <div className="flex items-start gap-3 px-5 py-2.5 border-b border-rule2 last:border-0">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-baseline gap-2">
          <span className="font-body font-medium text-ink text-[14px]">{f.name}</span>
          <span className="font-body text-ghost text-[12px]">{f.code}</span>
        </div>
        <div className="font-body text-ghost text-[12px] mt-0.5 leading-snug">{f.description}</div>
      </div>
      <span className={`font-body text-[12px] flex-shrink-0 ${statusColor}`}>{f.status}</span>
    </div>
  )
}
  ...
  return (
    <div className="flex items-center gap-3 px-5 py-2.5 border-b border-rule2 last:border-0">
      <div className="flex-1">
        <span className="font-body text-ink text-[14px]">{e.domain}</span>
      </div>
      <span className="font-body text-muted text-[13px] flex-1">{e.requirement}</span>
      {e.required
      ...
      {steps.map((s, i) => (
        <div key={i} className="flex items-start gap-3 px-5 py-2.5">
          ...
          <div className="flex-1 min-w-0 pb-1">
            <div className="font-body font-medium text-ink text-[13px]">{s.threshold}</div>
            <div className="font-body text-muted text-[12px] mt-0.5">{s.action}</div>
            <div className="font-body text-ghost text-[12px] mt-0.5">{s.channel}</div>
          </div>
        </div>
      ))}
    </div>
    ...
    <div className="flex h-full overflow-hidden content-reveal">
      ...
      <div className="w-[280px] flex-shrink-0 border-r border-rule2 flex flex-col bg-stone">
        <div className="flex-shrink-0 px-5 py-4 border-b border-rule2 bg-stone2">
          ...
          <div className="font-body text-muted text-[12px] mt-1.5 leading-relaxed">
        ...
        <div className="flex-shrink-0 px-5 py-3 border-b border-rule2 bg-warn/[0.04]">
          <div className="flex items-start gap-2">
            ...
            <div>
              <div className="font-body font-medium text-warn text-[12px]">Coverage gap detected</div>
              <div className="font-body text-ghost text-[12px] mt-0.5 leading-snug">{multiRegulatoryCoverage.coverageGap}</div>
            </div>
          </div>
        </div>
          ...
          {compliancePolicies.map(p => (
            <button key={p.id} type="button" onClick={() => setSelectedId(p.id)}
              ...
              <div className="flex items-start justify-between gap-2 mb-1">
                <span className={`font-display font-bold text-[15px] leading-none ${selectedId === p.id ? 'text-ink' : 'text-ink2'}`}>{p.name}</span>
                <StatusBadge status={p.status} />
              </div>
              <div className="font-body text-ghost text-[12px] mb-2">{p.jurisdiction}</div>
              <div className="flex items-center gap-2">
                <span className="font-body text-ghost text-[12px]">{p.frameworks.length} frameworks</span>
                {p.openItems.capaCount > 0 && (
                  <span className="font-body text-warn text-[12px]">· {p.openItems.capaCount} open CAPAs</span>
                )}
              </div>
            </button>
          ))}
        ...
        <div className="flex-shrink-0 px-5 py-3 border-t border-rule2 bg-stone2">
          <div className="font-body text-ghost text-[12px] tracking-normal mb-1.5">Export markets</div>
          <div className="flex flex-wrap gap-1">
            {multiRegulatoryCoverage.currentExportMarkets.map(m => (
              <span key={m} className="font-body text-ghost text-[12px] bg-stone3 px-1.5 py-0.5 border border-rule2">{m}</span>
            ))}
          </div>
        </div>
      </div>
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {policy.activeSince && <span className="font-body text-ghost text-[12px]">Active since {policy.activeSince}</span>}
                </div>
                <div className="font-display font-bold text-ink text-[24px] leading-none mb-1">{policy.name}</div>
                <div className="font-body text-muted text-[14px]">{policy.jurisdiction}</div>
              </div>
              <div className="flex gap-6 flex-shrink-0">
                {policy.nextInspection && (
                  <div className="text-right">
                    ...
                    <div className="font-body text-ghost text-[12px] tracking-normal mt-0.5">days to inspection</div>
                    <div className="font-body text-ghost text-[12px] mt-0.5">{policy.nextInspection.authority}</div>
                  </div>
                )}
                <div className="text-right">
                  ...
                  <div className="font-body text-ghost text-[12px] tracking-normal mt-0.5">open CAPAs</div>
                  {policy.openItems.overdueCount > 0 && (
                    <div className="font-body text-danger text-[12px] mt-0.5">{policy.openItems.overdueCount} overdue</div>
                  )}
                </div>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <div className="border-b border-rule2">
              <div className="px-5 py-2.5 bg-stone2 border-b border-rule2">
                <span className="font-body font-bold text-ink text-[13px]">Regulatory frameworks</span>
                <span className="ml-2 font-body text-ghost text-[12px]">{policy.frameworks.length} configured</span>
              </div>
            ...
            <div className="flex border-b border-rule2">
              ...
              <div className="flex-1 border-r border-rule2">
                <div className="px-5 py-2.5 bg-stone2 border-b border-rule2">
                  <span className="font-body font-bold text-ink text-[13px]">Evidence requirements</span>
                </div>
                <div className="divide-y divide-rule2">
                  <div className="flex items-center gap-3 px-5 py-2 bg-stone2/50">
                    <span className="font-body text-ghost text-[12px] tracking-normal flex-1">Domain</span>
                    <span className="font-body text-ghost text-[12px] tracking-normal flex-1">Requirement</span>
                    <span className="font-body text-ghost text-[12px] tracking-normal w-5">Req.</span>
                  </div>
              ...
              <div className="w-[320px] flex-shrink-0">
                <div className="px-5 py-2.5 bg-stone2 border-b border-rule2">
                  <span className="font-body font-bold text-ink text-[13px]">Escalation logic</span>
                </div>
            ...
            <div>
              <div className="px-5 py-2.5 bg-stone2 border-b border-rule2">
                <span className="font-body font-bold text-ink text-[13px]">Reporting templates</span>
              </div>
              {policy.reportingTemplates.map(t => (
                <div key={t.id} className="flex items-center gap-3 px-5 py-3 border-b border-rule2 last:border-0 hover:bg-stone2 transition-colors group">
                  <div className="flex-1">
                    <div className="font-body font-medium text-ink text-[14px]">{t.name}</div>
                    <div className="font-body text-ghost text-[12px] mt-0.5">{t.format}</div>
                  </div>
                  {t.lastGenerated
                    ? <span className="font-body text-ghost text-[12px]">Last: {t.lastGenerated}</span>
                    : <span className="font-body text-ghost text-[12px] italic">Never generated</span>
                  }
                  <button type="button"
                    className="font-body text-[12px] px-2.5 py-1.5 border border-rule2 text-muted hover:text-ink hover:border-ghost transition-colors opacity-0 group-hover:opacity-100">
                    Generate
                  </button>
                </div>
              ))}
            ...
            {policy.status === 'inactive' && (
              <div className="mx-6 my-6 px-5 py-4 border border-rule2 border-l-4 border-l-rule2 bg-stone2">
                <div className="font-body font-semibold text-ink text-[15px] mb-1">This policy is inactive</div>
                ...
                <button type="button"
                  className="font-body font-medium text-[13px] px-3.5 py-2 bg-ink text-stone hover:bg-ink/90 transition-colors">
                  Activate policy
                </button>
              </div>
            )}
