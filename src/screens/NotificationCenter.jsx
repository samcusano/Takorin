      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            <span className={`font-body italic font-medium text-[9px] px-1.5 py-px ${
            ...
            <span className="font-body italic text-muted text-[10px] px-1.5 py-px bg-stone3">{group.items.length}</span>
            <span className="font-body italic text-muted text-[10px]">{group.items[0].time}</span>
          </div>
        ...
        <span className="font-body italic text-ghost text-[9px] flex-shrink-0">{expanded ? '▴' : '▾'}</span>
      </button>
      ...
      <div className="flex items-start gap-3 px-4 py-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            ...
            <span className="font-body italic text-muted text-[10px]">{notif.time}</span>
          </div>
          <div className="font-body font-medium text-ink text-[12px] leading-snug">{notif.title}</div>
          <div className="font-body italic text-ghost text-[10px] mt-0.5 leading-relaxed">{notif.body}</div>
          {notif.linkedItem && <div className="font-body italic text-int text-[10px] mt-1">{notif.linkedItem}</div>}
          {confirmingDismiss && (
            <div className="flex items-center gap-2 mt-2 pt-2 border-t border-rule2 slide-in">
              <span className="font-body italic text-danger text-[10px] flex-1">Dismissing a safety override is logged. Confirm?</span>
              <button onClick={() => setConfirmingDismiss(false)} className="font-body italic text-[10px] px-2 py-1 text-muted hover:text-ink transition-colors">Cancel</button>
              <button onClick={() => { setConfirmingDismiss(false); onAction(notif.id, 'dismiss') }} className="font-body font-medium text-[10px] px-2.5 py-1 bg-danger/10 text-danger hover:bg-danger/20 transition-colors">Dismiss anyway</button>
            </div>
          )}
        </div>
        {!notif.dismissed && !confirmingDismiss ? (
          <div className="flex flex-col gap-1 flex-shrink-0">
            {notif.actionLabel && (
              <button onClick={() => onAction(notif.id, 'act')}
            ...
            <button onClick={handleDismiss}
              className="font-body italic text-[10px] px-2.5 py-1 text-ghost hover:text-muted transition-colors text-center">
              Dismiss
            </button>
          </div>
        ) : !notif.dismissed ? null : <span className="font-body italic text-ghost text-[9px] flex-shrink-0">Dismissed</span>}
      </div>
    </div>
  )
}
...
export default function NotificationCenter({ onClose }) {
  ...
  const content = (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex items-center justify-between px-5 py-3.5 flex-shrink-0" style={{ background: '#100F0D' }}>
        <div className="flex-1">
          <div className="font-display font-bold italic text-stone text-base leading-tight">{`Notification center — ${pending} pending`}</div>
          <div className="font-body italic text-stone/80 text-[12px] mt-1">J. Crocker · Plant Director · April 16, 2026</div>
        </div>
        {onClose && (
          <button onClick={onClose} className="ml-4 p-1.5 text-stone/60 hover:text-stone transition-colors flex-shrink-0" aria-label="Close notifications">
      ...
      <div className="flex border-b border-rule2 bg-stone2 px-4 py-2 gap-5 flex-shrink-0 flex-wrap">
        {[
          ...
          >
            <span className={`font-body italic text-[11px] transition-colors ${activeFilter === label ? 'text-ink' : 'text-muted'}`}>{label}</span>
            {count > 0 && <span className="font-body italic text-muted text-[10px] px-1.5 py-px bg-stone3">{count}</span>}
          </button>
        ))}
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {groupedItems.length === 0 && (
          <div className="px-4 py-8 text-center font-body italic text-ghost text-[12px]">No notifications{activeFilter !== 'All' ? ` in ${activeFilter}` : ''}.</div>
        )}
