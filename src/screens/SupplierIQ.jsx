    ...
    <>
      ...
      <aside className="fixed top-0 right-0 bottom-0 w-full max-w-[400px] bg-stone border-l border-rule2 z-50 flex flex-col slide-right">
        <div className="flex items-start justify-between px-5 py-4 border-b border-rule2 bg-stone2 flex-shrink-0">
          <div>
            <div className="font-body italic text-ghost text-[10px] mb-1">Certificate of Analysis</div>
            <div className="font-display font-bold italic text-ink text-base">{lot.ing}</div>
          </div>
          <button onClick={onClose} className="p-1 text-ghost hover:text-ink transition-colors">✕</button>
        </div>
        <div className="flex-1 overflow-y-auto p-5">
          {[
            ...
            <div key={r.l} className="flex justify-between py-2.5 border-b border-rule2 last:border-0">
              <span className="font-body italic text-ghost text-[11px]">{r.l}</span>
        ...
        <div className="px-5 py-3 border-t border-rule2 bg-stone2 flex-shrink-0">
          <button onClick={onClose} className="font-body text-[11px] px-3 py-1.5 bg-stone3 text-muted">Close</button>
        </div>
      </aside>
    </>
  )
...
function ShelfPill({ days, tone, useFirst }) {
  ...
  return (
    <div>
      <span className={`inline-flex items-center gap-1 font-body italic font-medium text-[11px] px-2 py-0.5 ${cls}`}>
      ...
      {useFirst && <div className="font-body italic text-warn text-[9px] mt-0.5">Use first</div>}
    </div>
  )
}
  ...
  return (
    <div className="flex items-start gap-3 py-3 border-b border-rule last:border-b-0">
      ...
      <div className="flex-1">
        <div className="font-body text-ghost text-[9px] uppercase tracking-wider mb-0.5">{label}</div>
        <div className="font-body font-medium text-ink text-[12px]">{name}</div>
        <div className="font-body italic text-ghost text-[10px]">{detail}</div>
        {gapMsg && (
          <div className="font-body italic text-danger text-[10px] mt-1 flex items-start gap-1">
            <span>⚠</span><span>{gapMsg}</span>
          </div>
        )}
        {onResolve && <button onClick={onResolve} className="font-body italic text-int text-[10px] mt-1 hover:underline">Go to Data Readiness →</button>}
      </div>
    </div>
  )
}
    ...
    <>
            ...
            <div key={i} className={`border-b border-rule last:border-b-0 ${audit?.needsAction ? 'bg-danger/[0.02]' : ''}`}>
              ...
              {audit && (
                <div className="px-4 pb-2 flex items-center gap-3 flex-wrap">
                  <span className="font-body italic text-ghost text-[9px]">Last audit: {audit.lastAudit}</span>
                  <span className={`font-body italic font-medium text-[9px] px-1 py-px ${audit.result === 'Approved' ? 'text-ok' : audit.result === 'Conditional' ? 'text-warn' : 'text-danger'}`}>{audit.result}</span>
                  {audit.findings > 0 && <span className="font-body italic text-ghost text-[9px]">{audit.findings} finding{audit.findings > 1 ? 's' : ''}</span>}
                  <span className="font-body italic text-ghost text-[9px]">Next: {audit.nextAudit}</span>
                  {audit.needsAction && <span className="font-body italic text-danger text-[9px]">Re-audit due</span>}
                </div>
              )}
            </div>
          )
      ...
      <SP title="FDA inspection" sub="FSMA 204">
        <div className="flex items-baseline gap-2 px-4 pt-3 pb-2">
          ...
          <span className="font-body italic text-ghost text-[11px]">days · Region 7 · Salina</span>
        </div>
        ...
        {d.fdaSteps.map((s, i) => (
          <div key={i} className="flex items-start gap-2.5 px-4 py-2.5 border-b border-rule last:border-b-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="font-body italic text-ghost text-[10px]">{s.sub}</div>
            </div>
            <span className={`font-body font-semibold text-[9px] flex-shrink-0 ${s.statusColor}`}>{s.status}</span>
          </div>
        ))}
      </SP>
        ...
        {d.gaps.map((g, i) => (
          <div key={i} className={`px-4 py-2.5 border-b border-rule last:border-b-0 border-l-2 ${
            ...
            <div className="flex justify-between items-start">
              ...
              <span className={`font-body font-semibold text-[9px] ml-2 flex-shrink-0 ${g.badgeColor}`}>{g.badge}</span>
            </div>
            <div className="font-body italic text-ghost text-[10px] mt-0.5">{g.sub}</div>
          </div>
        ))}
      </SP>
        ...
        {empResultsHistory.map((r, i) => (
          <div key={i} className={`flex items-center gap-2 px-4 py-2 border-b border-rule last:border-b-0 ${r.result === 'positive' ? 'bg-danger/[0.03]' : ''}`}>
            ...
            <div className="flex-1">
              ...
              <span className="font-body italic text-ghost text-[10px] ml-2">{r.zone} · {r.location}</span>
            </div>
            <span className={`font-body italic font-medium text-[9px] px-1.5 py-px ${r.result === 'positive' ? 'bg-danger/10 text-danger' : 'bg-ok/10 text-ok'}`}>
              {r.result === 'positive' ? `Pos ${r.cfu} CFU` : 'Neg'}
            </span>
            {r.capaId && <span className="font-body italic text-ghost text-[9px]">{r.capaId}</span>}
          </div>
        ))}
      </SP>
        ...
        <div className="px-4 py-3 space-y-2.5">
          {[{l:'Prior batch',v:'$0.42/unit',c:'text-ok'},{l:'This batch',v:'$0.48/unit',c:'text-danger'}].map((r,i)=>(
            <div key={i} className="flex justify-between items-baseline">
              <span className="font-body italic text-muted text-[12px]">{r.l}</span>
          ...
          <div className="font-body italic text-ghost text-[10px] border-t border-dashed border-rule2 pt-2">Driven by ingredient cost increases</div>
          {[{l:'Tomato sauce +14%',v:'−$0.04',pct:100},{l:'Canola oil +8%',v:'−$0.02',pct:57}].map((r,i)=>(
            <div key={i} className="flex items-center gap-2">
              <span className="font-body italic text-ghost text-[10px] flex-1">{r.l}</span>
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex border-b border-rule2 bg-stone2 px-4 py-2 gap-5 flex-shrink-0">
        {[['suppliers','Supplier scorecard'],['lots','Ingredient lots']].map(([key, label]) => (
          <button key={key} onClick={() => setActiveView(key)}
            ...
            <span className={`font-body italic text-[11px] transition-colors ${activeView === key ? 'text-ink' : 'text-muted'}`}>{label}</span>
          </button>
        ))}
      </div>
      ...
      <Layout side={side}>
                ...
                <div key={i} className={`border-b border-rule2 ${audit?.needsAction ? 'bg-danger/[0.02] border-l-2 border-l-danger' : 'border-l-2 border-l-transparent'}`}>
                  <div className="grid px-4 py-3" style={{ gridTemplateColumns:'28px 1fr 80px 100px 90px 100px' }}>
                    ...
                    <div>
                      ...
                      {audit?.reason && <div className="font-body italic text-warn text-[10px] mt-0.5">{audit.reason}</div>}
                    </div>
                    <div className="flex flex-col justify-center">
                      <span className={`font-body italic font-medium text-[9px] px-1.5 py-0.5 self-start ${
                        s.tierTone === 'ok' ? 'bg-ok/10 text-ok' : s.tierTone === 'danger' ? 'bg-danger/10 text-danger' : 'bg-int/10 text-int'
                      }`}>{s.tier}</span>
                    </div>
                    <div className="flex flex-col justify-center">
                      <div className="font-body italic text-ghost text-[9px]">Last audit</div>
                      <div className="font-body italic text-muted text-[10px]">{audit?.lastAudit}</div>
                    </div>
                    <div className="flex flex-col justify-center">
                      <div className="font-body italic text-ghost text-[9px]">Result</div>
                    ...
                    <div className="flex flex-col items-end justify-center">
                      ...
                      <div className="font-body italic text-ghost text-[9px]">score</div>
                    </div>
                  </div>
                  {audit?.needsAction && (
                    <div className="px-4 pb-3 flex gap-2">
                      ...
                      <button className="font-body italic text-[10px] px-3 py-1.5 text-muted hover:text-ink transition-colors">View audit history</button>
                    </div>
                  )}
                </div>
              )
            ...
            <thead>
              <tr className="border-b border-rule2 bg-stone2">
                {['Ingredient','Supplier · Lot','PO date','Shelf life','Delivery ETA','COA status','Action'].map(h => (
                  <th key={h} className="px-4 py-2 text-left font-body italic text-ghost text-[10px] font-normal">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {d.lots.map((lot, i) => (
                <tr key={i} className={`border-b border-rule2 hover:bg-stone2 transition-colors ${lot.urgent ? 'bg-danger/[0.03]' : ''}`}>
                  <td className="px-4 py-3 font-body font-medium text-ink"
                    style={lot.urgent ? { color:'#D94F2A' } : {}}>{lot.ing}</td>
                  <td className="px-4 py-3 font-body italic text-ghost text-[11px]">{lot.supplier}</td>
                  <td className="px-4 py-3 font-body italic text-ghost text-[11px]">{lot.po}</td>
                  ...
                  <td className="px-4 py-3">
                    ...
                    <div className="font-body italic text-ghost text-[10px] mt-0.5">{lot.deliveryTime}</div>
                  </td>
                  <td className="px-4 py-3">
        ...
        {coaRequested && (
          <div className="flex items-center gap-2 px-4 py-2.5 bg-ok/10 border-t border-ok/20 font-body italic text-ok text-[11px] slide-in">
        ...
        <div className="border-t border-rule2">
            ...
            <div className="flex gap-1 flex-shrink-0">
              {[{t:'ok',l:'CTE 1'},{t: namingResolved ? 'ok' : 'gap',l:'CTE 2'},{t:'pend',l:'CTE 3'},{t:'pend',l:'CTE 4'}].map((n,i) => (
                <div key={i} className="flex flex-col items-center gap-0.5">
                  ...
                  <span className="font-body italic text-ghost text-[8px]">{n.l}</span>
                </div>
              ))}
            </div>
            <p className="font-body italic text-[11px] flex-1">
              {namingResolved
          ...
          <div className="flex items-start gap-2 px-4 py-2.5 bg-ok/10 border-b border-rule2">
            ...
            <p className="font-body italic text-ink2 text-[11px]">FSMA 204 requires this chain to be submittable to FDA within <strong className="text-ok not-italic font-semibold">24 hours</strong> of a request. Auto-assembled from existing records.</p>
          </div>
        ...
        <div className="border-t border-rule2">
          ...
          <div className="grid grid-cols-2 divide-x divide-rule2">
            {[
              ...
              <div key={i} className="px-4 py-3">
                ...
                <div className={`font-body italic text-[10px] mt-0.5 ${r.tone === 'danger' ? 'text-danger' : 'text-warn'}`}>{r.impact}</div>
                <div className="font-body italic text-ghost text-[10px] mt-2 leading-relaxed">{r.detail}</div>
                {r.rfq && (
                  rfqSent
                    ? <div className="font-body italic text-ok text-[10px] mt-2">RFQ sent to ADM + Sysco · Response expected 48h</div>
                    : <button
          ...
          {rfqSent && (
            <div className="flex items-center gap-2 px-4 py-2.5 border-t border-rule2 bg-ok/10 font-body italic text-ok text-[11px] slide-in">
              <svg className="w-3 h-3 stroke-ok flex-shrink-0" fill="none" strokeWidth={2} viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>
