...
import { Urg, StatCell, SP, SPRow, SecHd, Btn, ConsequenceNotice, Layout, ActionBanner } from '../components/UI'
        ...
        <div className="flex">
          {events.map((e, i) => (
            <div key={i} className="flex flex-col items-center flex-1">
              <span className="font-body italic text-ghost text-[9px] h-6 flex items-end pb-1 leading-none">{e.time}</span>
              ...
              <div className="font-body italic text-ghost text-[9px] mt-0.5 text-center leading-tight px-1">{e.detail}</div>
            </div>
          ))}
        </div>
      </div>
  ...
  const side = (
    <>
      ...
      <SP title="Workforce development" sub="CAPA-driven">
        <div className="px-4 py-2 font-body italic text-warn text-[11px] border-b border-rule2 flex items-center gap-1.5">
          ...
          return (
            <div key={i} className="px-4 py-3 border-b border-rule last:border-b-0">
              <div className="flex items-start gap-2 mb-2">
                <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 font-display font-bold italic text-[11px] ${
                  op.fromCapa ? 'bg-warn/20 text-warn' : 'bg-stone3 text-muted'
                }`}>{op.initials}</div>
                <div className="flex-1 min-w-0">
                  <div className="font-body text-ink font-medium text-[12px]">{op.name}</div>
                  <div className="font-body italic text-ghost text-[10px]">{op.role}</div>
                </div>
                <button
                  onClick={() => !nominated[op.name] && op.badgeTone === 'warn' && setNominated(p => ({ ...p, [op.name]: true }))}
                  className={`font-body italic font-medium text-[10px] px-2 py-0.5 flex-shrink-0 transition-colors ${
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-body italic text-ghost text-[9px] whitespace-nowrap">{op.label}</span>
              </div>
              {op.fromCapa && <div className="font-body italic text-warn/80 text-[10px] mt-1">{op.fromCapa}</div>}
              {nominated[op.name] && !plan?.submitted && (
                <div className="mt-2 slide-in space-y-1.5">
                  <div className="font-body italic text-ghost text-[9px] uppercase tracking-widest">Training plan</div>
                  <select
                    value={form.level || ''}
              ...
              {plan?.submitted && !trainingCompletions[op.name] && (
                <div className="mt-2 space-y-1 slide-in">
                  <div className="flex items-center gap-1 font-body italic text-ok text-[10px]">
                  ...
                  {!completionForms[op.name] ? (
                    <button onClick={() => setCompletionForms(p => ({...p, [op.name]: { outcome:'', date:'', hours:'' }}))}
                      className="font-body italic text-int text-[10px] hover:underline">Mark complete →</button>
                  ) : (
                    <div className="space-y-1.5 slide-in">
                      ...
                      <div className="flex gap-1.5">
                        <input placeholder="Completion date" value={completionForms[op.name]?.date || ''} onChange={e => setCompletionForms(p => ({...p, [op.name]: {...p[op.name], date: e.target.value}}))}
                          className="flex-1 font-body italic text-ink text-[11px] bg-stone border border-rule px-2 py-1" />
                        <input placeholder="Hours" type="number" value={completionForms[op.name]?.hours || ''} onChange={e => setCompletionForms(p => ({...p, [op.name]: {...p[op.name], hours: e.target.value}}))}
                          className="w-16 font-body italic text-ink text-[11px] bg-stone border border-rule px-2 py-1" />
                      </div>
                      <button disabled={!completionForms[op.name]?.outcome || !completionForms[op.name]?.date}
                        onClick={() => {
              ...
              {trainingCompletions[op.name] && (
                <div className="flex items-center gap-1 mt-2 font-body italic text-ok text-[10px] slide-in">
          ...
          return (
            <div key={i} className={`flex items-start gap-2.5 px-4 py-2.5 border-b border-rule last:border-b-0 border-l-2 ${bg}`}>
              <div className="flex-1 min-w-0">
                ...
                <div className="font-body italic text-ghost text-[10px]">{c.cert} · {c.line}</div>
                {c.note && <div className={`font-body italic text-[10px] mt-0.5 ${color}`}>{c.note}</div>}
              </div>
              <div className="text-right flex-shrink-0">
                <div className={`display-num text-base ${color}`}>{c.expiresIn === 0 ? '0d' : `${c.expiresIn}d`}</div>
                <div className="font-body italic text-ghost text-[9px]">{c.expiresIn === 0 ? 'tonight' : 'remaining'}</div>
              </div>
            </div>
          )
        })}
      ...
      <SP title="Incoming supervisor" sub="PM shift · 14:02">
        <div className="px-4 py-3 space-y-1.5">
          ...
          <div className="font-body italic text-ghost text-[10px]">14:00–22:00 · Line 4</div>
          <div className="h-px bg-rule2 my-2" />
          <div className="font-body italic text-ink2 text-[11px] leading-relaxed">
        ...
        {timeToShift && (
          <div className="flex items-baseline gap-2 px-4 py-3 border-t border-rule2">
            ...
            <span className="font-body italic text-ghost text-[11px]">until PM shift starts</span>
          </div>
        )}
      </SP>
      ...
      <SP title="Recent handoffs" sub="Line 4">
        {d.lastHandoffs.map((h, i) => (
          <div key={i} className="flex justify-between px-4 py-2 border-b border-rule last:border-b-0 font-body text-[11px]">
            <span className="italic text-muted">{h.shift}</span>
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      {signed && handoffAccepted && (
        <div className="flex items-center gap-3 px-5 py-3 bg-ok/10 border-b border-ok/20 flex-shrink-0">
          ...
          <span className="font-body italic text-ok text-[12px]">
      ...
      <Layout side={side}>
        ...
        <div className="border-b border-rule2">
          ...
          {[
            ...
            <div key={i} className={`flex gap-3 px-4 py-3.5 border-b border-rule2 last:border-b-0 border-l-2 ${i === 0 ? 'border-l-brass bg-brass/[0.03]' : 'border-l-ok bg-ok/[0.02]'}`}>
              ...
              <div>
                ...
                <div className="font-body italic text-ink2 text-[11px] leading-relaxed mb-1.5">
                  {i === 0
                ...
                <div className="font-body italic text-[10px] leading-relaxed px-2 py-1.5 bg-stone3 border-l-2 border-l-warn text-muted mb-2">
                ...
                {!operatorAcknowledgments[op.name] ? (
                  ...
                  <div className="flex items-center gap-1 font-body italic text-ok text-[10px] slide-in">
                    <svg className="w-3 h-3 stroke-ok flex-shrink-0" fill="none" strokeWidth={2} viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>
          ...
          <div className="px-4 py-3 border-t border-rule2">
            {!showNearMiss && !nearMissSubmitted && (
              <button onClick={() => setShowNearMiss(true)} className="font-body italic text-ghost text-[11px] hover:text-muted transition-colors">+ Log a near-miss</button>
            )}
            {showNearMiss && !nearMissSubmitted && (
              <div className="slide-in space-y-2">
                ...
                <textarea placeholder="What happened?" value={nearMissForm.what} onChange={e => setNearMissForm(p => ({...p, what: e.target.value}))}
                  className="w-full font-body italic text-ink text-[11px] bg-stone border border-rule px-2 py-1 h-14 resize-none" />
                ...
                <div className="flex gap-2">
                  <button disabled={!nearMissForm.station || !nearMissForm.what}
            ...
            {nearMissSubmitted && (
              <div className="font-body italic text-ok text-[11px] slide-in">Near-miss logged · CAPA created and assigned to supervisor</div>
            )}
          </div>
        </div>
            ...
            <div key={i} className={`border-l-2 border-b border-rule2 last:border-b-0 ${
              ...
              <div className="grid grid-cols-[28px_1fr]">
                ...
                <div className="p-4 pl-2 space-y-1.5">
                  ...
                  <p className="font-body italic text-ink2 text-[12px] leading-relaxed">{c.desc}</p>
                  {c.evidence && <p className="font-body italic text-ghost text-[11px]">▸ {c.evidence}</p>}
                  {c.events && c.events.map((e, j) => (
                    <div key={j} className="flex gap-2 font-body text-[11px]">
                      <span className="italic text-ghost flex-shrink-0">{e.time}</span>
        ...
        <div>
          <div className="flex items-center border-b border-rule2">
            ...
            <button onClick={() => setShowSanitationForm(p => !p)}
              className="px-4 font-body italic text-ghost text-[10px] hover:text-muted transition-colors flex-shrink-0">+ Log entry</button>
          </div>
          {showSanitationForm && (
            <div className="px-4 py-3 border-b border-rule2 bg-stone2 space-y-2 slide-in">
              <div className="font-body italic text-ghost text-[9px] uppercase tracking-widest">New sanitation log</div>
              <div className="flex gap-2">
                <select value={sanitationForm.line} onChange={e => setSanitationForm(p => ({...p, line: e.target.value}))}
                  className="flex-1 font-body italic text-ink text-[11px] bg-stone border border-rule px-2 py-1 cursor-pointer">
                ...
                <select value={sanitationForm.shift} onChange={e => setSanitationForm(p => ({...p, shift: e.target.value}))}
                  className="w-20 font-body italic text-ink text-[11px] bg-stone border border-rule px-2 py-1 cursor-pointer">
                  <option value="">Shift</option><option>AM</option><option>PM</option>
                </select>
                <div className="flex items-center gap-1 font-body italic text-ghost text-[11px]">
                  <input type="number" value={sanitationForm.checks} onChange={e => setSanitationForm(p => ({...p, checks: e.target.value}))}
                    className="w-10 font-body italic text-ink text-[11px] bg-stone border border-rule px-2 py-1 text-center" />
                  <span>/</span>
              ...
              <input placeholder="Notes (if partial)" value={sanitationForm.notes} onChange={e => setSanitationForm(p => ({...p, notes: e.target.value}))}
                className="w-full font-body italic text-ink text-[11px] bg-stone border border-rule px-2 py-1" />
              <div className="flex gap-2">
                <button disabled={!sanitationForm.line || !sanitationForm.shift}
                  onClick={() => {
                ...
                <button onClick={() => setShowSanitationForm(false)} className="font-body italic text-[11px] text-ghost px-2">Cancel</button>
              </div>
            </div>
          )}
          {sanitationEntries.map((e, i) => (
            <div key={i} className="grid border-b border-rule2 bg-ok/[0.02]" style={{ gridTemplateColumns:'1fr 80px 80px 80px 120px' }}>
              <div className="px-4 py-2.5">
                ...
                <div className="font-body italic text-ghost text-[11px]">{e.tech} — {e.time}</div>
              </div>
              <div className="flex items-center justify-center font-body italic text-ghost text-[11px]">{e.checks}/{e.total}</div>
              <div className="flex items-center px-2 font-body italic text-ghost text-[11px]">{e.time}</div>
              <div className="flex items-center px-2 font-body italic text-ghost text-[11px]">100%</div>
              <div className="flex items-center justify-center">
                <span className="font-body italic font-medium text-[10px] px-2 py-0.5 bg-ok/10 text-ok">Complete</span>
              </div>
            </div>
          ))}
              ...
              <thead>
                <tr className="border-b border-rule2 bg-stone2">
                  {['Line · Shift','Tech','Status','Checks',''].map(h => (
                    <th key={h} className="px-4 py-2 text-left font-body italic text-ghost text-[9px] font-normal">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {sanitationLogs.map((log, i) => (
                  <tr key={i} className={`border-b border-rule2 last:border-b-0 ${log.status === 'gap' ? 'bg-danger/[0.03]' : ''}`}>
                    ...
                    <td className="px-4 py-2.5 font-body italic text-ghost">{log.tech}</td>
                    ...
                    <td className="px-4 py-2.5 font-body italic text-ghost">{log.checks}/{log.total}</td>
                    <td className="px-4 py-2.5">
                      {log.capaId && (
                        <span className="font-body italic text-warn text-[10px]">{log.capaId}</span>
                      )}
                    </td>
                  </tr>
                ))}
          ...
          <div className="px-4 py-2 font-body italic text-ghost text-[10px] border-t border-rule2">
          ...
          {scheduleData.days.flatMap(day => day.conflicts.map((issue, j) => ({ date: day.date, issue, key: `${day.date}-${j}` }))).map(c => (
            <div key={c.key} className="flex items-start gap-2 px-4 py-2 border-b border-rule2 bg-danger/[0.03]">
              ...
              <div>
                ...
                <div className="font-body italic text-ghost text-[10px] mt-0.5">{c.issue}</div>
              </div>
            </div>
          ))}
          {scheduleData.days.map((day, di) => (
            <div key={di} className="border-b border-rule2 last:border-b-0">
              <div className="flex items-center gap-2 px-4 py-2 bg-stone2 border-b border-rule2">
                ...
                <span className="font-body italic text-ghost text-[10px]">{day.label}</span>
                {day.conflicts.length > 0 && <span className="ml-auto font-body italic text-danger text-[9px]">⚠ conflict</span>}
              </div>
              {day.shifts.map((sh, si) => (
                <div key={si} className="flex gap-3 px-4 py-2.5 border-b border-rule last:border-b-0">
                  <div className="w-28 flex-shrink-0">
                    <div className="font-body italic text-ghost text-[10px]">{sh.time}</div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {sh.crew.map(c => {
                      ...
                      return (
                        <span key={c} className={`font-body italic text-[10px] px-1.5 py-0.5 ${conflict ? 'bg-danger/10 text-danger' : fatigued ? 'bg-warn/10 text-warn' : 'bg-stone3 text-muted'}`}>
        ...
        {!signed && (
          <div className="px-5 py-4 bg-stone3/50 border-t border-rule2">
            <div className="font-body italic text-ink2 text-[12px] mb-3 leading-relaxed">
        ...
        {signed && handoffAccepted && (
          <div className="px-5 py-3 bg-ok/10 border-t border-rule2 flex items-center gap-3">
            ...
            <div className="font-body italic text-ok text-[12px]">
