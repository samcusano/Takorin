...
function healthState(unit, ext) {
  ...
  if (unit.status === 'maintenance') return {
    key: 'pm', label: 'In PM',
    color: 'text-muted', bg: 'bg-stone2', border: 'border-l-rule2',
...
function UnitCard({ unit, selected, onSelect }) {
  ...
  return (
    ...
    >
      ...
      {/* Task */}
      <div className="font-body text-label text-muted mb-3 leading-snug line-clamp-2 min-h-[28px]">
      ...
      {/* Autonomy mode */}
      <div className={`font-body text-label ${AUTONOMY_COLOR[mode]} mb-2`}>
        {AUTONOMY_LABEL[mode]}
      </div>

      {/* Exception signals — only if relevant */}
      {exception && (
        <div className={`flex items-start gap-1.5 font-body text-label leading-snug ${unit.alert.type === 'danger' ? 'text-danger' : 'text-warn'}`}>
...
function TraceRow({ time, text }) {
  return (
    <div className="flex items-start gap-3 py-2.5 border-b border-rule2 last:border-0">
      <span className="font-body text-muted text-label flex-shrink-0 w-9 pt-px">{time}</span>
      <span className="font-body text-ink text-label leading-snug">{text}</span>
    </div>
  )
}
      ...
      <div className="-mx-5 px-5 py-4 border-b border-rule2 mb-4">
        ...
        {!rs ? (
          ...
          <>
            ...
            {rs.risk && (
              <div className="flex items-start gap-1.5 font-body text-danger text-label mb-3">
                <AlertTriangle size={9} className="flex-shrink-0 mt-0.5" strokeWidth={2.5} />
                <span>{rs.risk}</span>
              </div>
            )}
