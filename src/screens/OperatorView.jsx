...
import { SecHd, Urg, SP, ActionBanner } from '../components/UI'
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex border-b border-rule2 bg-stone2 flex-shrink-0">
        {OPERATORS.map(o => (
          <button key={o.name}
            ...
            <div className={`w-7 h-7 rounded-full flex items-center justify-center font-display font-bold italic text-[11px] ${selected === o.name ? 'bg-ochre/20 text-ochre' : 'bg-stone3 text-muted'}`}>
              {o.initials}
            </div>
            <div className="text-left">
              <div className="font-body font-medium text-ink text-[13px]">{o.name}</div>
              <div className="font-body italic text-ghost text-[11px]">{o.role.split('·')[0].trim()}</div>
            </div>
          </button>
        ))}
      </div>
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        <div className="flex-1 overflow-y-auto">
          ...
          {myTasks.length === 0 ? (
            <div className="px-4 py-4 font-body italic text-ghost text-[12px]">No tasks yet — tasks assigned by your supervisor in ShiftIQ will appear here.</div>
          ) : myTasks.map((t, i) => (
            <div key={i} className={`flex items-center gap-3 px-4 py-4 border-b border-rule2 last:border-b-0 ${t.done ? 'opacity-50' : ''}`}>
              ...
              <div className="flex-1">
                ...
                {t.dueTime && <div className="font-body italic text-ghost text-[11px]">Due {t.dueTime}</div>}
              </div>
            </div>
          ))}
          ...
          <div className="border-t border-rule2">
            ...
            <div className="px-4 py-4 border-b border-rule2 border-l-2 border-l-warn bg-warn/[0.02]">
              <div className="font-body italic text-ink2 text-[14px] leading-relaxed">
                {SAFETY_CONTEXT[selected]}
              </div>
            </div>
            <div className="px-4 py-3 font-body italic text-ghost text-[11px]">
              ...
              {myFlags.map((f, i) => (
                <div key={i} className="flex items-start gap-2.5 px-4 py-3.5 border-b border-rule2 last:border-b-0 bg-warn/[0.02]">
                  ...
                  <div>
                    ...
                    <div className="font-body italic text-warn text-[11px]">{f.reason}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="border-t border-rule2">
              ...
              {myNearMisses.map((n, i) => (
                <div key={i} className="px-4 py-3.5 border-b border-rule2 last:border-b-0">
                  ...
                  <div className="font-body italic text-ink2 text-[12px] mt-0.5">{n.what}</div>
                  {n.action && <div className="font-body italic text-ok text-[11px] mt-0.5">Corrective step: {n.action}</div>}
                </div>
              ))}
            </div>
          )}
        ...
        <div className="w-[260px] flex-shrink-0 border-l border-rule2 overflow-y-auto bg-stone2">
          <SP title="Certification progress" sub={op?.role}>
            <div className="px-4 py-3">
              <div className="font-body italic text-ghost text-[10px] mb-1.5">{op?.certLabel}</div>
              ...
              <div className="font-body italic text-ghost text-[10px] mt-1">toward next certification level</div>
            </div>
          </SP>
            ...
            {!plan?.submitted ? (
              ...
              <div className="px-4 py-3 space-y-2">
                <div className="flex justify-between items-baseline">
                  <span className="font-body italic text-ghost text-[10px]">Level</span>
                  <span className="font-body font-medium text-ink text-[12px]">{plan.level}</span>
                </div>
                <div className="flex justify-between items-baseline">
                  <span className="font-body italic text-ghost text-[10px]">Trainer</span>
                  <span className="font-body font-medium text-ink text-[12px]">{plan.trainer}</span>
                </div>
                <div className="flex justify-between items-baseline">
                  <span className="font-body italic text-ghost text-[10px]">Starts</span>
                  <span className="font-body font-medium text-ink text-[12px]">{plan.startDate}</span>
                </div>
                {completion ? (
                  <div className={`flex items-center gap-1 pt-2 border-t border-rule font-body italic text-[11px] ${completion.outcome === 'Passed' ? 'text-ok' : 'text-warn'}`}>
                  ...
                  <div className="flex items-center gap-1 pt-2 border-t border-rule font-body italic text-ghost text-[10px]">
          ...
          <SP title="Cert expiry" sub="Your certifications">
            <div className="px-4 py-3">
              {selected === 'C. Reyes' && <div className="font-body italic text-ghost text-[11px]">L1 Pack Line · No expiry scheduled yet</div>}
              {selected === 'P. Okonkwo' && <div className="font-body italic text-ghost text-[11px]">L2 Topping · Valid through Dec 2026</div>}
              {selected === 'F. Adeyemi' && (
                <div>
                  <div className="font-body italic text-warn text-[11px]">L2 QA Inspector · Expires in 35 days</div>
                  <div className="font-body italic text-ghost text-[10px] mt-1">Renewal scheduled — check with supervisor</div>
                </div>
              )}
            </div>
          </SP>
