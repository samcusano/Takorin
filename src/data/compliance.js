// ── Compliance policy data — Shift 2: Policy-based compliance engine ──────────
// Compliance is a configurable policy layer, not a set of hardcoded US screens.

export const compliancePolicies = [
  {
    id: 'fda-us',
    name: 'FDA / FSMA 204',
    jurisdiction: 'United States',
    status: 'active',
    activeSince: '2024-01-01',
    color: 'text-ok',
    bg: 'bg-ok/[0.05]',
    border: 'border-l-ok',
    frameworks: [
      { id: 'haccp',   name: 'HACCP',        code: '21 CFR 120.7', status: 'active',   description: 'Hazard Analysis & Critical Control Points' },
      { id: 'gmp',     name: 'GMP',          code: '21 CFR 110',   status: 'active',   description: 'Good Manufacturing Practice — personnel, sanitation, equipment' },
      { id: 'fsma204', name: 'FSMA 204',     code: 'Section 204',  status: 'active',   description: 'Food traceability — lot-level supply chain documentation' },
      { id: 'sqf',     name: 'SQF Level 2',  code: 'SQF 9.0',      status: 'active',   description: 'Supplier quality management and audit protocol' },
    ],
    evidenceRequirements: [
      { domain: 'CAPA closure',      requirement: 'Digital upload (PDF/image)', required: true  },
      { domain: 'Checklist signing', requirement: 'Digital signature — timestamped', required: true },
      { domain: 'Lot traceability',  requirement: 'FSMA 204 Key Data Elements', required: true  },
      { domain: 'Supplier COA',      requirement: 'Lab-certified, within 90 days', required: true  },
      { domain: 'CCP log',           requirement: 'Real-time digital entry, no gaps', required: true },
    ],
    escalationLogic: [
      { threshold: '24h overdue', action: 'Notify assigned owner + supervisor', channel: 'In-app + email' },
      { threshold: '48h overdue', action: 'Escalate to Plant Director',         channel: 'In-app + email + SMS' },
      { threshold: '72h overdue', action: 'Auto-generate legal hold notification', channel: 'All channels + audit log' },
    ],
    reportingTemplates: [
      { id: 'fsma-export', name: 'FSMA 204 Audit Package', format: 'PDF + structured JSON', lastGenerated: '2026-05-10' },
      { id: 'fda-capa',    name: 'FDA CAPA Register',       format: 'PDF',                  lastGenerated: '2026-05-14' },
    ],
    openItems: { capaCount: 14, overdueCount: 2, blockers: 1 },
    nextInspection: { date: '2026-06-03', daysRemaining: 18, authority: 'FDA Region 7' },
  },
  {
    id: 'gb-china',
    name: 'GB Standards / SAMR',
    jurisdiction: 'China (Domestic)',
    status: 'inactive',
    activeSince: null,
    color: 'text-ghost',
    bg: 'bg-stone2',
    border: 'border-l-rule2',
    frameworks: [
      { id: 'gb2760',   name: 'GB 2760-2024',  code: 'GB 2760',   status: 'inactive', description: 'General Standard for Use of Food Additives — effective Feb 2025' },
      { id: 'gb22000',  name: 'ISO 22000 / GB', code: 'GB/T 22000',status: 'inactive', description: 'Food safety management systems — Chinese national adoption' },
      { id: 'fsl42',    name: 'Food Safety Law', code: 'Art. 42',  status: 'inactive', description: 'Mandatory lot-level traceability per China FSL Article 42' },
      { id: 'samr',     name: 'SAMR Registration', code: 'SAMR',  status: 'inactive', description: 'State Administration for Market Regulation — production license' },
    ],
    evidenceRequirements: [
      { domain: 'CAPA closure',      requirement: 'Paper + digital (dual requirement)', required: true  },
      { domain: 'Checklist signing',  requirement: 'Physical signature + digital scan', required: true },
      { domain: 'Lot traceability',   requirement: 'QR code-based digital traceability', required: true },
      { domain: 'Supplier COA',       requirement: 'GB-certified laboratory',           required: true  },
      { domain: 'Annual inspection',  requirement: 'SAMR production license renewal',   required: true },
    ],
    escalationLogic: [
      { threshold: '48h overdue', action: 'Notify factory manager', channel: 'In-app + WeChat Work' },
      { threshold: '72h overdue', action: 'Escalate to compliance officer + SAMR draft', channel: 'All channels' },
    ],
    reportingTemplates: [
      { id: 'samr-annual', name: 'SAMR Annual Production Report', format: 'Structured XML (SAMR portal)', lastGenerated: null },
      { id: 'gb-lot',      name: 'GB Lot Traceability Chain',      format: 'QR-linked JSON',              lastGenerated: null },
    ],
    openItems: { capaCount: 0, overdueCount: 0, blockers: 0 },
    nextInspection: null,
  },
  {
    id: 'eu-ifs',
    name: 'IFS Food Standard',
    jurisdiction: 'European Union',
    status: 'monitoring',
    activeSince: null,
    color: 'text-ochre',
    bg: 'bg-ochre/[0.04]',
    border: 'border-l-ochre',
    frameworks: [
      { id: 'ifs8',   name: 'IFS Food 8',        code: 'IFS v8.0', status: 'monitoring', description: 'International Featured Standards — food safety and quality' },
      { id: 'ec852',  name: 'EC 852/2004',        code: 'EC 852',   status: 'monitoring', description: 'EU hygiene regulation for foodstuffs' },
      { id: 'efsa',   name: 'EFSA Traceability',  code: 'Reg. 178', status: 'monitoring', description: 'Farm-to-fork traceability mandate (Article 18)' },
    ],
    evidenceRequirements: [
      { domain: 'CAPA closure',     requirement: 'CAPA report + root cause analysis', required: true },
      { domain: 'Allergen control', requirement: 'Validated cleaning verification',   required: true },
      { domain: 'Lot traceability', requirement: 'One-up / one-down traceability',    required: true },
    ],
    escalationLogic: [
      { threshold: '24h overdue', action: 'Notify QA lead', channel: 'In-app + email' },
      { threshold: '7 days overdue', action: 'IFS corrective action protocol initiated', channel: 'All channels' },
    ],
    reportingTemplates: [
      { id: 'ifs-audit', name: 'IFS Pre-Audit Package', format: 'PDF', lastGenerated: null },
    ],
    openItems: { capaCount: 0, overdueCount: 0, blockers: 0 },
    nextInspection: null,
  },
  {
    id: 'iso22000',
    name: 'ISO 22000',
    jurisdiction: 'Global',
    status: 'monitoring',
    activeSince: null,
    color: 'text-int',
    bg: 'bg-int/[0.04]',
    border: 'border-l-int',
    frameworks: [
      { id: 'iso22000', name: 'ISO 22000:2018', code: 'ISO 22000', status: 'monitoring', description: 'Food safety management systems — international standard' },
      { id: 'iso9001',  name: 'ISO 9001:2015',  code: 'ISO 9001',  status: 'monitoring', description: 'Quality management system — process control' },
    ],
    evidenceRequirements: [
      { domain: 'CAPA closure',      requirement: 'Effectiveness verification required', required: true },
      { domain: 'Internal audit',    requirement: 'Annual management review',            required: true },
      { domain: 'Supplier control',  requirement: 'Approved supplier list + audit',      required: true },
    ],
    escalationLogic: [
      { threshold: '48h overdue', action: 'Notify quality manager', channel: 'In-app + email' },
    ],
    reportingTemplates: [
      { id: 'iso-mgmt', name: 'Management Review Report', format: 'PDF', lastGenerated: null },
    ],
    openItems: { capaCount: 0, overdueCount: 0, blockers: 0 },
    nextInspection: null,
  },
]

export const multiRegulatoryCoverage = {
  currentExportMarkets: ['United States', 'Japan', 'South Korea', 'Germany', 'Australia'],
  domesticMarket: 'China',
  activePolicies: 1,
  monitoringPolicies: 2,
  inactivePolicies: 1,
  coverageGap: 'GB Standards / SAMR required for domestic market — currently inactive',
}
